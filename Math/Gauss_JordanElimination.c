...
int Gaussian_JordanElimination(double a[][10], int r, int c, int printSteps);
...
int main()
{
	...
	Gaussian_JordanElimination(augMat, r, c, 1);
	printMatrix(augMat, r, c);

	return 0;
}

int Gaussian_JordanElimination(double augMat[][10], int r, int c, int printSteps)
{
	...
	for(int rowX=0; rowX<r; rowX++)
	{
		...
		//make the pivot 1
		k = augMat[rowX][fixedCol];
		for(int i=fixedCol; i<c; i++)
		{
			augMat[rowX][i] = augMat[rowX][i] / k;
		}

		for(int rowY = rowX+1; rowY<r; rowY++)
		{

			k = -augMat[rowY][fixedCol];
		...
		for(int rowY = rowX-1; rowY>=0; rowY--)
		{
			k = -augMat[rowY][fixedCol];
			replaceRow(augMat, rowY, rowX, k, c);
			if(printSteps)
